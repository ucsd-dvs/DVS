...
public Crescent_info getCrescent() {
	  ...
	  this.erodeAndDilate(crescent_mat, ring_removed_mat, new Mat());		// This erosion & dilation is used for the "ring" removal 
  ...
  public Mat erodeAndDilate(Mat src, Mat dst, Mat kernel) {
	  ...
	  Imshow interImg = new Imshow("E");
	  Imshow resultImg = new Imshow("F");
	  Mat inter = new Mat();
	  Mat inter2 = new Mat();
	  
	  Imgproc.erode(src, inter, kernel);
  ...
  public Mat getSkinThreshold(Mat src, Mat dst) {
	  ...
	  double THH = 0.075 * max;    // High threshold for hue
